...
migrations: ## Make migrations
	docker exec -it aituring_web python3 manage.py makemigrations
migrate: migrations ## Run migrate
	docker exec -it aituring_web python3 manage.py migrate
shell: shell ## Run migrate
	docker exec -it aituring_web python3 manage.py shell
superuser: ## Create django superuser
	docker exec -it aituring_web python3 manage.py createsuperuser
dumpdata: ## regenerate fixture over all models
	docker exec -it aituring_web python3 manage.py dump_limited_data
bulk-loaddata: ## regenerate fixture over all models
	docker exec -it aituring_web python3 manage.py loaddata tests/fixtures/json/AuthUser.json tests/fixtures/json/*.json
clean:
...
pyenv-create-env:
	pyenv virtualenv -f 3.11 aituring_env
pyenv-init-env:
	pyenv shell aituring_env
	pip install poetry
...
drop-db: ## drop all tables in database
	docker exec -it aituring_postgres psql -U postgres -d aituring_db -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public;"
collectstatic:
	docker exec -it aituring_web python3 manage.py collectstatic --noinput
